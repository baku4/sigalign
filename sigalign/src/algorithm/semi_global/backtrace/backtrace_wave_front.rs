...
impl WaveFront {
    ...
    pub fn get_extended_length_of_endpoint(
        ...
        let wave_front_score = &self.wave_front_scores[penalty as usize]; //.components_by_k[index_of_component as usize].m;
        let max_k = wave_front_score.max_k;
        let k = index_of_component as i32 - max_k;
        let fr = wave_front_score.components_by_k[index_of_component as usize].m.fr;
        let query_extended_length = fr - k; // FR - K
        let target_extended_length = fr;
        (query_extended_length, target_extended_length)
    }
    pub fn backtrace_from_endpoint(
        &self,
        mut penalty: u32,
        index_of_component: u32,
        penalties: &Penalty,
        ...
        let mut reversed_operations: Vec<AlignmentOperations> = Vec::new(); // TODO: Capacity can be applied?
                        ...
                        BackTraceMarker::FromM => {
                            ...
                            if match_count == 0 {
                                if let Some(AlignmentOperations {
                                    ...
                                    count: last_fr
                                }) = reversed_operations.last_mut() {
                                    *last_fr += 1;
                                } else {
                                    reversed_operations.push(
                                        AlignmentOperations {
                                            operation: AlignmentOperation::Subst,
                                            count: 1
                                        }
