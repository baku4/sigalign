...
fn local_alignment(
    ...
    sorted_anchor_indices.into_iter().for_each(|current_anchor_index| {
        ...
        if !current_anchor.registered {
            ...
            left_traversed_anchors.iter().rev().for_each(|traversed_anchors| {
                symbol.push(traversed_anchors.anchor_index.clone())
            });
            symbol.push(current_anchor_index.clone());
            right_traversed_anchors.iter().for_each(|traversed_anchors| {
                symbol.push(traversed_anchors.anchor_index.clone())
            });
            ...
            // For left symbols
            for index in (0..left_traversed_anchors.len()).rev() {
                ...
                if left_anchor.registered {
                    ...
                    if left_anchor.extensions_cache.is_none() {
                        ...
                        // TODO: Extend right
                        left_anchor.extensions_cache = None;
                    };
                    ...
                    if search_result.is_ok() {
                        let leftmost_symbol_index = left_traversed_anchors.len() - index - 1;
            ...
            // For right symbols
            for index in (0..right_traversed_anchors.len()).rev() {
                ...
                if right_anchor.registered {
                    ...
                    if right_anchor.extensions_cache.is_none() {
                        ...
                        // TODO: Extend left
                        right_anchor.extensions_cache = None;
                    };
                    ...
                    if search_result.is_ok() {
                        let rightmost_symbol_index = left_traversed_anchors.len() + 1 + index;
...
impl PosTable {
    fn extend_with_right_first(
        ...
        let left_vpc_vector = left_wave_front.get_sorted_vpc_vector(cutoff.maximum_penalty_per_scale);

        //
        // (4) Find optimal position of VPC vectors
        //
        let (left_vpc_index, right_vpc_index) = VPC::get_optimal_position(&left_vpc_vector, &right_vpc_vector, cutoff.maximum_penalty_per_scale, anchor_size);
        ...
        //
        let left_scaled_penalty_margin_of_extension = ((left_extension.length * cutoff.maximum_penalty_per_scale) - (left_extension.penalty * PRECISION_SCALE)) as i64;
        ...
        let left_scaled_penalty_margins: Vec<i64> = left_traversed_anchors.iter().map(|traversed_anchor| {
            let remained_scaled_penalty_margin = ((traversed_anchor.remained_length * cutoff.maximum_penalty_per_scale) - (traversed_anchor.remained_penalty * PRECISION_SCALE)) as i64;

            left_max_scaled_penalty_margin + remained_scaled_penalty_margin - left_scaled_penalty_margin_of_extension
        }).collect();

        let right_scaled_penalty_margin_of_extension = ((right_extension.length * cutoff.maximum_penalty_per_scale) - (right_extension.penalty * PRECISION_SCALE)) as i64;
...
impl VPC {
    ...
    fn get_optimal_position(
        ...
        let anchor_scaled_penalty_margin = (anchor_size * maximum_penalty_per_scale) as i64;
...
impl WaveFront {
    ...
    fn get_sorted_vpc_vector(&self, maximum_penalty_per_scale: usize) -> Vec<VPC> {
        ...
        let mut sorted_vpc_vector: Vec<VPC> = vec![VPC::new_of_anchor_end()];
