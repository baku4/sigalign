...
impl AnchorsPreset {
    ...
    fn create_anchors_by_patterns(
        ...
        let estimation_per_pattern = EstimationPerPattern::new(self.total_pattern_count, pattern_size, cutoff, min_penalty_for_pattern, matched_pattern_index_list);
...
impl AnchorsByPattern {
    fn new(
        ...
        estimation_per_pattern: &EstimationPerPattern,
    ) -> Self {
        ...
        let anchors: Vec<Anchor> = record_positions.into_iter().map(|record_position| {
            Anchor {
                ...
                record_position,
                size: pattern_size,
                left_estimation: estimation_per_pattern.estimations[pattern_index].clone(),
                left_extension: None,
...
impl EstimationPerPattern {
    fn new(
        ...
        let estimations: Vec<Estimation> = start_index_of_pattern.into_iter().enumerate().map(|(end_index, start_index)| {
