...
impl<'a> SequenceProvider<'a> for InMemorySequences {
    fn len(&self) -> usize {
        self.records.len()
    }
...
/// [super::SequenceRecords] that stores sequences on disk and fetch data for each request.
#[derive(Debug, Serialize, Deserialize, PartialEq)]
pub struct OnDiskSeqRecs {
