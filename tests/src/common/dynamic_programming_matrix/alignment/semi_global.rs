...
pub fn dp_semi_global_to_pattern_existing_targets(
    query: &[u8],
    ...
    for target_index in target_indices {
        ...
        let alignments = parse_valid_semi_global_result_from_dpm(
...
pub fn dp_semi_global_to_ref_file(
    query: &[u8],
    ref_file: &PathBuf,
    mismatch_penalty: u32,
    gap_open_penalty: u32,
    ...
    while let Some(mut record) = ref_reader.next() {
        ...
        let alignments = dp_semi_global_to_target(
            query,
            &target_buffer,
            mismatch_penalty,
            gap_open_penalty,
...
pub fn dp_semi_global_to_target(
    query: &[u8],
    target: &[u8],
    mismatch_penalty: u32,
    gap_open_penalty: u32,
    ...
    let alignments = parse_valid_semi_global_result_from_dpm(&dp_matrix, min_length, max_penalty_per_length);
