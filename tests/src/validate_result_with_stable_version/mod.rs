...
#[test]
fn print_the_first_alignment_result_for_semi_global() {
    init_logger();
    info!("Integrated test of current algorithms starts");
    ...
    // Perform alignment
    let [semi_global_result_answer, _] = get_answer_or_generate().unwrap();
    let semi_global_result_answer = convert_result_of_stable_version_to_current(&semi_global_result_answer);
    info!("Answers are loaded");
    ...
    // To view result
    //
    let result_index_count = semi_global_result_answer.0.len();
    ...
    for result_index in result_index_list {
        ...
        let one_read = semi_global_result_answer.0[result_index].read.clone();
        let mut one_semi_global_result_answer = semi_global_result_answer.0[result_index].result.clone();
        one_semi_global_result_answer.0.sort_by_key(|x| x.index);
        ...
        if !right_result_includes_left(&one_semi_global_result_answer, &one_semi_global_result_of_current) {
            // println!(
            //     "{:#?}",
            ...
            println!(
                ...
                res_count(&one_semi_global_result_answer),
                res_count(&one_semi_global_result_of_current),
            );
        }
